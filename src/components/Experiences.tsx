...
const experiences: Experience[] = [
  {
    ...
    logo: "https://tse1.mm.bing.net/th/id/OIP.qcy4L6R4F5ShRSoOSzobUAHaHa?rs=1&pid=ImgDetMain&o=7&rm=3",
  ...
  <div className={`relative ${size} aspect-square shrink-0 z-10`}>
    ...
    {logo ? (
      <img
        src={logo}
        alt={company}
        className={`relative rounded-full aspect-square border-2 border-black dark:border-white ${size} bg-white object-contain p-2`}
      />
    ) : (
                ...
                <div className="flex-1 min-w-0 pb-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-3 sm:mt-4">
                    {exp.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-0.5 sm:py-1 text-xs sm:text-sm border border-dashed border-gray-400 dark:border-gray-500 rounded-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800/60 hover:border-blue-400 dark:hover:border-blue-500 transition-colors"
                      >
                        {techIcons[tech] && (
                          <img src={techIcons[tech]} alt={tech} className="w-3.5 h-3.5 sm:w-4 sm:h-4 object-contain" />
                        )}
                        {tech}
                      </span>
                    ))}
